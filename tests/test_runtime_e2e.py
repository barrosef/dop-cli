...
from dop.runtime.e2e import (
    ...
    next_run_number, E2E_SUITE_ORDER,
)
...
def test_resolve_target_suite():
    result = resolve_e2e_target("optum-support-fe", known_suites=["optum-support-fe", "providers-front-end"])
    assert result == {"kind": "suite", "suites": ["optum-support-fe"], "filter": None}

def test_resolve_target_jira():
    result = resolve_e2e_target("OG-150", known_suites=["optum-support-fe", "providers-front-end"])
...
def test_find_suites_for_jira(tmp_path):
    ...
    (suite1 / "test_og_150_save.py").touch()
    ...
    found = find_suites_for_jira("og_150", e2e_root=tmp_path,
                                  suites=["optum-support-fe", "providers-front-end"])
...
def test_suite_order():
    assert E2E_SUITE_ORDER == ["optum-support-fe", "providers-front-end", "Canal-empresa-fe"]
